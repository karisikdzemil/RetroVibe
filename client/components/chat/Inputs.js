...
const Inputs = ({ user, socket, setChats, typeOfChat }) => {
  const [input, setInput] = useState("");
  const uploadInput = useRef(null);

  const sendMessage = () => {
    if (input) {
      const msg = { content: input, type: "text", user, topic: typeOfChat };
      socket.emit("send_message", msg);
      setChats((prev) => ({
        ...prev,
        [typeOfChat]: [...(prev[typeOfChat] || []), msg],
      }));
  ...
  const handleImageUpload = (e) => {
    ...
    if (file && (file.type === "image/jpeg" || file.type === "image/png")) {
      ...
      const msg = { content: img, type: "image", user, topic: typeOfChat };
      setChats((prev) => ({
        ...prev,
        [typeOfChat]: [...(prev[typeOfChat] || []), msg],
      }));
