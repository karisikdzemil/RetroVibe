  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center">
      <div className="text-center grid grid-rows-3 gap-2 gradient p-8 rounded-md">
        ...
        />
        <button onClick={addUser} disabled={!input} className={`text-xl w-full text-white font-bold py-2 px-3 rounded-md ${input ? 'bg-sky-400' : 'bg-slate-400'}`}>Join Chat</button>
      </div>
    </div>
  );
};
